...
"/* Fix: icons must NOT use position:absolute/fixed at root level */" &
...
"/* Cluster bubbles */" &
".marker-cluster-small,.marker-cluster-medium,.marker-cluster-large{background:rgba(30,58,95,0.18)!important;}" &
".marker-cluster-small div,.marker-cluster-medium div,.marker-cluster-large div{background:rgba(30,58,95,0.85)!important;color:white!important;font-family:Segoe UI,sans-serif;font-weight:700;font-size:13px;}" &
...
"</div>" &
"<div class='legend'><div class='legend-title'>Location Type</div><div class='li'><div class='ld ld-plant'></div>Manufacturing Plant</div><div class='li'><div class='ld ld-wh'></div>Warehouse</div></div>" &
...
"function updateCnt(){document.getElementById('cnt').textContent=allMarkers.filter(function(x){return map.hasLayer(x.marker)||mcg.hasLayer(x.marker);}).length;}" &
...
"if(x.type===t){" &
"if(typeOn[t]){mcg.addLayer(x.marker);}else{mcg.removeLayer(x.marker);}" &
"}});" &
"updateCnt();}" &
